...
describe('siteBuildRunner', () => {
  ...
  describe('Expected Worker Output', () => {
    ...
    const worker = new QueueWorker(queueName, connection, (job) =>
      jobProcessors.siteBuildRunner(job, testJobOptions),
    );
    ...
    it(`should fail the job and build when
        CF tasks status goes beyond the
        max number of attempts`, async () => {
      const guid = 'task-guid';
      const taskState = 'PENDING';

      sinon.stub(CloudFoundryAPIClient.prototype, 'startSiteBuildTask').resolves({
        guid,
        state: taskState,
      });
      ...
      const message = taskMessage(build.id);
      sinon.stub(SiteBuildQueue, 'setupTaskEnv').resolves({
        build,
        message,
      });
