...
describe('Build API', () => {
  ...
  describe('POST /v0/build', () => {
    ...
    describe('unsuccessful requests', () => {
      ...
      it(`returns a 404 if a build to restart
          is not associated with the site`, async () => {
      ...
      it(`should render a 403
          if the user is not associated with the given site`, async () => {
        const user = await factory.user();
    ...
    describe('successful requests', () => {
      describe('with an existing build', () => {
        ...
        it(`should create a new build
            for the site given an existing build id`, async () => {
          ...
          const response = await validCreateRequest(csrfToken.getToken(), cookie, {
            buildId: reqBuild.id,
            siteId: site.id,
          });
        ...
        it(`should NOT create a new build
            if a branch build already exists @state=queued`, async () => {
          await reqBuild.update({
            state: 'queued',
          });
          const response = await validCreateRequest(csrfToken.getToken(), cookie, {
            buildId: reqBuild.id,
        ...
        it(`should NOT create a new build
            if a branch build already exists @state=created`, async () => {
          await reqBuild.update({
            state: 'created',
          });
          const response = await validCreateRequest(csrfToken.getToken(), cookie, {
  ...
  describe('GET /v0/site/:site_id/build', () => {
    it('should require authentication', (done) => {
      factory
        .site()
        .then((site) => request(app).get(`/v0/site/${site.id}/build`).expect(403))
        .then((response) => {
    ...
    it('should list builds for a site associated with the current user', async () => {
      const { site, user } = await createSiteUserOrg();
      const cookie = await authenticatedSession(user);
      ...
      builds.forEach((build) => {
        const responseBuild = response.body.find(
    ...
    it(`should not list builds for a site
        not associated with the current user`, async () => {
      const { site, user } = await createSiteUserOrg();
      const cookie = await authenticatedSession(user);
    ...
    it("shouldn't list more than 100 builds", async () => {
      const { site, user } = await createSiteUserOrg();
      const cookie = await authenticatedSession(user);

      await factory.bulkBuild(
        {
          site: site.id,
          user: user.id,
          username: user.username,
    ...
    it('should not display unfound build', async () => {
      const { site, user } = await createSiteUserOrg();
      const cookie = await authenticatedSession(user);
      await Promise.all([
        factory.build({
  ...
  describe('GET /v0/build/:id', () => {
    it('should require authentication', (done) => {
      factory
        .build()
        .then((build) => request(app).get(`/v0/build/${build.id}`).expect(403))
        .then((response) => {
    ...
    it('should return a single build matching the given build id', async () => {
      const { site, user } = await createSiteUserOrg();
      const cookie = await authenticatedSession(user);
    ...
    it(`should not return the matching build
        if it is not associated with the current user`, (done) => {
      const userPromise = factory.user();
    ...
    it('should not display unfound build', (done) => {
      ...
      Promise.props({
        ...
        .then((promisedValues) => {
          ...
          return request(app).get('/v0/build/-1000').set('Cookie', cookie).expect(404);
        })
        .then((response) => {
  ...
  describe('POST /v0/build/:id/status/:token', () => {
    ...
    it("should report the build's status back to github", async () => {
      ...
      const fetchContentStub = sinon
        .stub(GithubBuildHelper, 'fetchContent')
        .resolves('{}');
    ...
    it(`should report the build's success status
        back to github w/o build settings file`, async () => {
      const statusNock = githubAPINocks.status({
        state: 'success',
        commitSha: clonedCommitSha,
      });
    ...
    it(`should respond with a 403
        and not modify the build for an invalid build token`, (done) => {
      let build;
  ...
  describe('POST /v0/build/:id/metrics/:token', () => {
    ...
    it(`should respond with a 200
        and modify the build for a correct build token`, (done) => {
      let build;
