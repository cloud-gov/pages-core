...
describe('Site Branch Config model', () => {
  ...
  it('`branch` cannot be null when context is not `preview`', async () => {
    const instance = SiteBranchConfig.build({ context: 'site' });
    const error = await instance.validate().catch(e => e);

    expect(error).to.be.an('Error');
    expect(error.name).to.eq('SequelizeValidationError');
    expect(error.errors.map(e => e.message)).to.include('Branch attribute cannot be null when context attribute is not preview');
  });

  it('`s3Key` cannot be null when context is not `preview`', async () => {
    const instance = SiteBranchConfig.build({ branch: 'test', context: 'test' });
    const error = await instance.validate().catch(e => e);

    expect(error).to.be.an('Error');
    expect(error.name).to.eq('SequelizeValidationError');
    expect(error.errors.map(e => e.message)).to.include('The s3Key attribute cannot be null when context attribute is not preview');
  });
  ...
  it('should create a valid instance with a branch and config', async () => {
    ...
    const instance = SiteBranchConfig.build({ branch, config });
    const validation = await instance.validate();
  ...
  it('should create a valid instance with a branch, config, and is not a preview', async () => {
    const branch = 'test-1'
    const s3Key = 'site/test/repo'
    const config = { test: "config" }
    const context = 'site'
    const instance = SiteBranchConfig.build({ branch, config, s3Key, context });
    const validation = await instance.validate();

    expect(validation.dataValues).to.have.keys(['id', 'branch', 's3Key', 'config', 'context']);
    expect(validation.dataValues.branch).to.equal(branch);
    expect(validation.dataValues.config).to.equal(config);
  ...
  describe('.siteScope()', () => {
    it('returns the site-branch-config by site id', async () => {
      ...
      const siteBranchConfigBySiteId = await SiteBranchConfig.siteScope(site.id)
        .findAll()
  ...
  describe('.getConfig()', () => {
    ...
    it('should return null when there is no matching branch and no site preview config', async () => {
      ...
      await SiteBranchConfig.create({ siteId: site.id, branch, config, s3Key, context: 'site' })
    ...
    it('should return the record for the config with the branch specified', async () => {
      ...
      const branch = 'main'
      const config = { test: 'config' }
      const previewConfig = { test: 'preview' }
      const context = 'site'

      const site = await Factory.site({}, { noSiteBranchConfig: true });
      await SiteBranchConfig.create({ siteId: site.id, branch, config, s3Key, context })
      await SiteBranchConfig.create({ siteId: site.id, context: 'preview', config: previewConfig })
    ...
    it('should return the preview record for the config if specified branch is not found', async () => {
      const s3Key = 'test/s3/path'
      const branch = 'main'
      const previewBranch = 'preview'
      const config = { test: 'config' }
      ...
      const site = await Factory.site({}, { noSiteBranchConfig: true });
      await SiteBranchConfig.create({ siteId: site.id, branch, config, s3Key, context: false })
      await SiteBranchConfig.create({ siteId: site.id, context: previewContext, config: previewConfig })
