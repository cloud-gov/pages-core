...
describe('SiteUser model', () => {
  it('returns the site object with user association', (done) => {
    ...
    .then((user) => {
      user1 = user;
      return factory.site({ users: [user1] });
    })
    .then((model) => {
      site1 = model;
    ...
    .then((siteUser) => {
      ...
      expect(json.buildNotificationSettings).to.equal('site');
    ...
    .then((siteWithUsers) => {
      ...
      return Promise.all([
        User.findOne({ where: { id: user1.id }, include: [Site] }),
        User.findOne({ where: { id: user2.id }, include: [Site] }),
      ]);
    })
    .then((usersWithSites) => {
