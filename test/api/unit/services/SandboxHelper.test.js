...
const {
  notifyOrganizations,
  cleanSandboxes,
} = require('../../../../api/services/SandboxHelper');
  ...
  const verifyOrgsVsArgs = async (orgIds, spyArgs) => {
    ...
    spyArgs
      ...
      .map((o) =>
        expect(
          orgsToNotify.map((n) =>
            n.Users.filter((x) => x.OrganizationRole.roleId === managerRole.id).map(
              (u) => u.id,
            ),
          ),
    ...
    spyArgs
      .flat()
      .map((o) =>
        expect(orgsToNotify.map((n) => n.Sites.map((u) => u.id))).to.eql(
          o.Sites.map((v) => v.id),
        ),
      );
  };
...
describe('cleanSandboxes', () => {
  ...
  it('should clean all sites for since last cleaned', async () => {
    ...
    expect(moment(org.sandboxNextCleaningAt).format('YYYY-MM-DD')).to.equal(
      moment(now).subtract(1, 'day').format('YYYY-MM-DD'),
    );
    await cleanSandboxes(now);
    org.Sites.forEach((s) => expect(destroyerStub.args).to.deep.include([s]));
    await org.reload();
    expect(moment(org.sandboxNextCleaningAt).format('YYYY-MM-DD')).to.equal(
      moment(now).add(sandboxDays, 'days').format('YYYY-MM-DD'),
    );
  });
  ...
  it('should fail to clean all sites', async () => {
    ...
    orgs.forEach((o) =>
      o.Sites.forEach((s) => expect(destroyerStub.args).to.deep.include([s])),
    );
  });
});
