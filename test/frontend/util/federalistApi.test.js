...
function testRouteCalled(routeName, { method = 'GET', body } = {}) {
  ...
  expect(fetchMock).route(routeName).to.have.been.called.with.options(expectedOptions);
}

describe('federalistApi', () => {
  before(() => {
    fetchMock.get(
      `${API}/site/${testSite.id}/build`,
      { build: true },
      { name: 'getBuilds' },
    );
    fetchMock.get(
      `${API}/build/${testBuild.id}/log/offset/${buildLogOffset}`,
      { log: true },
      {
        name: 'getBuildLogs',
      },
    ...
    fetchMock.get(
      ...
      { files: [] },
      {
        name: 'getPublishedFilesWithQueryParam',
      },
    );
    ...
    fetchMock.put(`${API}/site/5`, 400, {
      name: 'putSiteError',
    });
    fetchMock.get(`${API}/site/6/user-environment-variable`, [uev], {
      name: 'fetchUserEnvironmentVariables',
    });
    fetchMock.post(
      `${API}/site/6/user-environment-variable`,
      {},
      {
    ...
    fetchMock.delete(
      `${API}/site/6/user-environment-variable/8`,
      {},
      {
        name: 'deleteUserEnvironmentVariable',
      },
    );
    fetchMock.get(`${API}/site/8/basic-auth`, credentials, {
      name: 'fetchBasicAuth',
    });
    fetchMock.post(`${API}/site/8/basic-auth`, credentials, {
      name: 'saveBasicAuth',
    });
    fetchMock.delete(
      `${API}/site/8/basic-auth`,
      {},
      {
        name: 'removeBasicAuth',
      },
    );
  });
  ...
  it('defines updateUserSettings', () => {
    const body = {
      buildNotificationSettings: { 3: 'none' },
    };
  ...
  it('defines createUserEnvironmentVariable', () => {
    ...
    testRouteCalled('createUserEnvironmentVariable', {
      method: 'POST',
      body: {},
    });
  });
