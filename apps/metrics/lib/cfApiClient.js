...
class CloudFoundryAPIClient {
  ...
  fetchOrganizationByName(orgName) {
    return this.authRequest('GET', `/v3/organizations?names=${orgName}`)
      .then(res => res.resources[0]);
  }

  fetchUsageSummary(orgGuid) {
    return this.authRequest('GET', `/v3/organizations/${orgGuid}/usage_summary`)
      .then(res => res.usage_summary);
  }

  fetchUsageSummaryByOrgName(orgName) {
    return this.fetchOrganizationByName(orgName)
      .then(org => this.fetchUsageSummary(org.guid));
  }

  fetchOrganizationQuota(orgQuotaGuid) {
