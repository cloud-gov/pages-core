...
const cmdRevertSiteSubdomain = site => `UPDATE site SET subdomain = '${generateS3ServiceName(site.owner, site.repository)}' WHERE id = ${site.id}`;
...
exports.down = (db, callback) => db.runSql(getSites)
    .then((sites) => Promise.all(sites.rows.map(site => db.runSql(cmdRevertSiteSubdomain(site)))))
    .then(() => callback())
    .catch(callback);
