  ...
  return {
    ...
    ...((serviceInstanceGuid || spaceGuid || servicePlanGuid) && {
      relationships: {
        ...
        ...(servicePlanGuid && createRelationship('service_plan', servicePlanGuid)),
      },
    }),
    ...(parameters && {
      parameters,
...
class CloudFoundryAPIClient {
  ...
  async startBuildTask(
    ...
    // construct the task parameter template by
    // filling in values from the BuildTaskType metadata
    ...
    try {
      return await this.authRequest('POST', `/v3/apps/${appGUID}/tasks`, taskParams);
    } catch (error) {
      if (attempt === totalAttempts) {
        throw error;
      }
   ...
   * @param {Array} [message.containerName]
   * - The name of the app task container to build the site
   ...
   * @param {string} params.cfDomainWithCdnPlanGuid
   * The guuid of the external domain service plan
   * @returns
   */
  ...
  createS3ServiceInstance(name, serviceName, spaceGuid) {
    return this.fetchS3ServicePlanGUID(serviceName).then((servicePlanGuid) => {
      ...
      return this.accessToken().then((token) =>
        this.request('POST', '/v3/service_instances', token, body),
      );
    });
  }
  ...
  createServiceKey(serviceInstanceName, serviceInstanceGuid, keyIdentifier = 'key') {
    ...
    return this.accessToken().then((token) =>
      this.request('POST', '/v3/service_credential_bindings', token, body),
    );
  }

  createSiteBucket(name, spaceGuid, keyIdentifier = 'key', serviceName = 'basic-vpc') {
    return this.createS3ServiceInstance(name, serviceName, spaceGuid)
      ...
      .then(() =>
        this.retry('fetchCredentialBindingsInstance', `${name}-${keyIdentifier}`),
      );
  }
    ...
    return this.accessToken()
      ...
      .then((entity) =>
        this.accessToken().then((token) =>
          this.request('DELETE', `/v3/routes/${entity.guid}`, token),
        ),
      );
  }

  deleteServiceInstance(name) {
    return this.fetchServiceInstance(name).then((instance) =>
      this.accessToken().then((token) =>
        this.request('DELETE', `/v3/service_instances/${instance.guid}`, token).then(
          () => ({
            guid: instance.guid,
          }),
        ),
  ...
  deleteServiceInstanceCredentials(guid) {
    ...
    return this.accessToken().then((token) => this.request('DELETE', endpoint, token));
  }
  ...
  fetchServiceInstanceCredentials(name) {
    return this.accessToken()
      ...
      .then(({ guid }) =>
        this.accessToken().then((token) =>
          this.request('GET', `/v3/service_credential_bindings/${guid}/details`, token),
        ),
      )
  ...
  fetchS3ServicePlanGUID(serviceName) {
    return this.accessToken()
      .then((token) =>
        this.request('GET', `/v3/service_plans?names=${serviceName}`, token),
      )
  ...
  async fetchTaskAppGUID(appName) {
    return this.accessToken()
      .then((token) => this.request('GET', `/v3/apps/?names=${appName}`, token))
  ...
  async retry(
    ...
    if (!res || ['initial', 'in progress'].includes(res?.last_operation?.state)) {
