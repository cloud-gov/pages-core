...
function sandboxReminder({
  ...
  const sitesList = sites.map(s => `<li style="${css.p}"><a style="${css.a}" target="_blank" href="${hostname}/sites/${s.id}/builds">${s.owner}/${s.repository}</a></li>`).join('\n');
  return layout(`
    <p style="${css.p}">
      Your 90 days are nearly up! The contents of your free sandbox organization <a href="${organizationLink}" style="${css.a}" target="_blank">${organizationName}</a> will be deleted on ${dateStr}. This includes all of your previews, build logs, and site settings for the following sites:
    </p>
    <ul>
      ${sitesList}
    </ul>
    <p style="${css.p}">
      While your Pages sandbox will be emptied on this date, your GitHub code repositories will remain unchanged.
    </p>
    ...
    <p style="${css.p}">
      ...
      <a href="mailto:pages-inquiries@cloud.gov" style="${css.a}" target="_blank">pages-inquiries@cloud.gov.</a>
    </p>
