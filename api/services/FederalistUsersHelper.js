...
const audit18F = ({ auditorUsername, fedUserTeams }) => {
  ...
  return User.findOne({ where: { username: auditorUsername } })
    ...
    .then((admins) => {
      ...
      return Promise.all(fedUserTeams.map(team => GitHub.getTeamMembers(auditor.githubAccessToken, team)));
    })
    .then((teams) => {
      const removed = [];
...
const federalistUsersAdmins = auditorUsername => User.findOne({ where: { username: auditorUsername } })
