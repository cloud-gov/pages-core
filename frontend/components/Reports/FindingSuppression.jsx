...
function FindingSuppression({
  ...
  function handleUpdate() {
    return api.updateSiteBuildTask(siteId, sbtId, { rules: customerRules })
    ...
    <>
      { !suppressed && willBeSuppressed && (
        <p className="font-body-sm text-italic">
          ...
          {' '}
          <button type="button" onClick={() => deleteRule()} className="usa-link usa-button--unstyled text-italic">Cancel suppression.</button>
        </p>
      )}
      { !suppressed && !willBeSuppressed && (
        <p className="font-body-sm text-italic">
          False positive?
          {' '}
          <button type="button" onClick={() => addNewRule()} className="usa-link usa-button--unstyled text-italic">
            Click here
          </button>
          {' '}
      ...
      {suppressed && (
        <section className="usa-alert usa-alert--info usa-alert--suppressed padding-y-1 margin-top-3">
          <div className="usa-alert__body">
            ...
            { suppressedBy !== 'Pages' && willBeSuppressed && (
              <p className="font-body-sm text-italic">
                ...
                {' '}
                <button type="button" onClick={() => deleteRule()} className="usa-link usa-button--unstyled text-italic">Click here</button>
                {' '}
