...
describe('LocationBar Component', () => {
  ...
  it('renders root directory correctly, in text (not link) if current directory', () => {
    ...
    expect(screen.getByText(`${domain}~assets`)).toBeInTheDocument();
    expect(
      screen.queryByRole('link', { name: `${domain}~assets` }),
  ...
  it('renders single-level folder correctly, with parent as link', () => {
    ...
    expect(screen.getByRole('link', { name: `${domain}~assets` })).toBeInTheDocument();
  ...
  test('clicking domain/~assets resets to root if it is a link', () => {
    ...
    fireEvent.click(screen.getByRole('link', { name: `${domain}~assets` }));
