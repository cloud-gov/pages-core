...
const mapSites = (organizations, siteData, user) =>
  siteData
    ...
    .map((site) => {
      ...
      return (
        <SiteListItem key={site.id} organization={organization} site={site} user={user} />
      );
    });
...
const getSites = (organizations, sites, user) => {
  ...
  return <ul className="usa-card-group">{mapSites(organizations, data, user)}</ul>;
};
  ...
  return (
    <div className="grid-col-12">
      ...
      <div className="grid-row margin-y-3">
        <div className="grid-col">{getSites(organizations, groupedSites, user)}</div>
      </div>
    </div>
  );
};
