...
function Reports() {
  ...
  const filteredScans = scans?.filter(
    (scan) => !buildIdToFilterBy || scan.buildId === buildIdToFilterBy,
  );
  ...
  return (
    <div>
        ...
        <>
          <div>
            <p className="font-body-sm line-height-sans-4 measure-6 margin-bottom-4">
              Pages is now offering monthly Automated Site Reports, which examine your
              Pages site for common website issues and provide guidance and resources for
              remediation. You can request an immediate report for any recent site branch
              from <Link to={`/sites/${id}/builds`}>Build history</Link>. You can also
              customize your report results in{' '}
              <Link to={`/sites/${id}/settings`}>Site settings</Link>. For more
              information on Pages Automated Site Reports, check out the{' '}
              <a
                target="_blank"
            ...
            {siteBuildTasks.length > 0 && (
              <>
                <h3 className="font-sans-lg">
                  Available report types ({siteBuildTasks.length})
                </h3>
                {siteBuildTasks.map((task) => (
                  <ExpandableArea bordered title={task.name} key={task.id}>
                    <div className="well">
                      <p>
                        Next scheduled for&nbsp;
      ...
      <p>
        ...
        <a
          href="mailto:pages-support@cloud.gov?subject=Site%Reports%20feedback"
          target="_blank"
          rel="noreferrer"
        >
