...
const infoContent = (
  <>
    The domain <strong>CANNOT</strong> be edited since it has already been provisioned.
    Please reach out to the Pages support team at{' '}
    <a
...
function EditCustomDomain() {
  ...
  const { availableConfigs, domain, setDomainValues, updateSiteDomain } = useSiteDomain(
    id,
    domainId,
  );
  ...
  if (domain.data.state !== 'pending') {
    return (
      <>
        ...
        <div>
          <AlertBanner status="warning" message={infoContent} alertRole={false} />
        </div>
      </>
    );
  }
