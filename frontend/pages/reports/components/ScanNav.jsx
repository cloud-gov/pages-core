...
const ScanNav = ({ groups = [], generated, buildId, siteId, showBackButton = false }) => (
    ...
    >
      ...
      <thead>
        <tr className="height-5">
          ...
          <th className="text-right" scope="col" role="columnheader">
            Count
          </th>
        </tr>
      </thead>
      <tbody>
        {groups.map(({ label, color, count = 0, usePill = false, boldMe = false }) => (
          <tr className="height-5" key={label}>
            <th scope="col">
              {usePill ? (
                ...
                <span className={boldMe ? 'text-bold' : undefined}>{label}</span>
              )}
              <span className="usa-sr-only">{plural(count, 'findings')},</span>
            </th>
            <td className="font-mono-sm text-tabular text-right line-height-body-3">
              <span className={boldMe ? 'text-bold' : undefined}>{count}</span>
            </td>
          </tr>
        ))}
      </tbody>
